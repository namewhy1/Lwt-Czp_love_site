        ...
        // ============ 云端同步（Vercel KV）===========
        // 注意：此 token 不要写在代码里，必须在 Vercel 项目环境变量里设置 ADMIN_TOKEN。
        // 在本地预览时也可以在浏览器 localStorage 手动放一个 admin_token 用于测试。
        ...
        function addEggSticker() {
            if (easterEggConfig.stickers.length < 5) { // Limit stickers for UI reasons
        ...
        function saveEggSettings() {
            ...
            // Toggles are already updated in the object
        ...
        function loadEggSettings() {
            ...
            if (saved) {
                const s = JSON.parse(saved);
                // Merge with default to avoid breaking on new fields
